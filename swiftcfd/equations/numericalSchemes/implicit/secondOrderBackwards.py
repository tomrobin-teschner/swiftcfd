...
class SecondOrderBackwards(NumericalSchemesBase):
    ...
    def _compute_interior(self, direction, block_id, solver, var_name):
        ...
        inv_2dt = self.coefficients[block_id]['b']
        if self.coefficients[block_id]['is_second_order']:
            for (i, j) in self.mesh.internal_loop_single_block(block_id):
                b_index = self.mesh.map3Dto1D(block_id, i, j)
                b_value = 4.0 * inv_2dt * self.field_manager.fields[var_name].old[block_id, i, j] - \
                    inv_2dt * self.field_manager.fields[var_name].oldold[block_id, i, j]
                solver.add_to_b(b_index, b_value)
        else:
            for (i, j) in self.mesh.internal_loop_single_block(block_id):
                b_index = self.mesh.map3Dto1D(block_id, i, j)
                b_value = inv_2dt * self.field_manager.fields[var_name].old[block_id, i, j]
                solver.add_to_b(b_index, b_value)
